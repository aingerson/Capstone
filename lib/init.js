                    ...
                    if (d >= 0 && d < p.allSigWords.length) {
                        ...
                        if (!isInList(word, foundAll) && word != "" && word != currWord) {
                            if (!isEdge(currWord, word)) { //no edge already exists
                                ...
                                var thisEdge = getTreeEdge(currWord, word); //already an edge, remember the poem you found it in for later
                                if(thisEdge==null) thisEdge = getGraphEdge(currWord,word);
...
function research(word1,word2,state){
      for (var i = 0; i < poemList.length; i++) { //go through every poem
      var poemTitle = poemList[i];
      var p = poemList[i].data('poem'); //grab its respective poem
  for (var d = j - dist; d <= j + dist; d++) {
      if (d >= 0 && d < p.allSigWords.length) {
          ...
          if (!isEdge(word1, word2)) {
            graphEdges.push(new Edge(word1, word2, poemTitle)); //add it to the overall list of edges
          }
          else {
                  var thisEdge = getEdge(word1, word2); //already an edge, remember the poem you found it in for later
                  thisEdge.addPoem(poemTitle);
              }
            }
          else{
...
function showConnections(word1, word2) {
    var e = getTreeEdge(word1, word2);
    if(e==null) e = getGraphEdge(word1,word2);
    if (e == null) return;
                    ...
                    if (d >= 0 && d < p.allSigWords.length) {
                        ...
                        if (!isInList(word, foundAll) && word != "" && word != currWord) {
                            if (!isEdge(currWord, word)) {
                                ...
                                var thisEdge = getTreeEdge(currWord, word);
                                if(thisEdge==null) thisEdge=getGraphEdge(currWord,word);
                                thisEdge.addPoem(poemTitle);
                            }
                        }
 ...
 */
function getTreeEdge(word1, word2) {
...
function getGraphEdge(word1,word2){
  for (var g = 0; g < graphEdges.length; g++) {
      if (graphEdges[g] == null) continue;
      if ((graphEdges[g].w1 == word1 && graphEdges[g].w2 == word2) || (graphEdges[g].w1 == word2 && graphEdges[g].w2 == word1)) {
          return graphEdges[g];
      }
  }
  return null;
