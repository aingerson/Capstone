/*
 * Draws the tree saved in root (global)
 */
function makeTree() {
    $("#tree").empty(); //clear current tree
    var nodes;
    var totalNodes = 0;
...
// Helper functions for collapsing and expanding nodes.
    ...
    /*
     * Searches the tree in order to re-build the edges array
     */
    function searchTree(start){
      //TODO this
      if (typeof(start.children) == 'undefined') {
          start.children = [];
          return;
      ...
      for(var p=0;p<start.children.length;p++){
        searchTree(start.children[p]);
        research(start.name,start.children[p].name,stateTree);
      }
    }

    /*
     * Deletes from a particular index.
     */
    ...
    /*
     * Returns true if node is parent of specified node
     */
...
/*
 * Returns which mode is currently active
 */
